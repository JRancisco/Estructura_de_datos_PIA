...
conn = sqlite3.connect('renta_de_bicicletas.db')
cursor = conn.cursor()
...
def solicitar_entero_rodada(mensaje):
    while True:
        try:
            valor = int(input(mensaje))
            if valor in (20, 26, 29):
                return valor
            else:
                print("Rodada inválida. Debe ser 20, 26 o 29.")
        except ValueError:
...
def registrar_unidad():
    ...
    while not rodada_valida:  
        rodada = solicitar_entero("Ingrese la rodada (20, 26, 29): ")
    ...
    try:
        ...
        print("Error en la inserción: asegúrese de que los datos cumplen con las restricciones.")
